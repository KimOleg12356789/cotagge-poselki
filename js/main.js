...
document.addEventListener('DOMContentLoaded', function() {
    const track = document.querySelector('.reviews-video__slider');
    const cards = Array.from(track.children);
    const prevButton = document.querySelector('.reviews-prev');
    const nextButton = document.querySelector('.reviews-next');
    let currentIndex = 0;

    // Функция для расчета ширины карточек
    function calculateCardWidth() {
        const containerWidth = document.querySelector('.reviews-video__slider-container').offsetWidth;
    ...
    // Обновление состояния карточек и ширины
    function updateCards() {
        const { cardWidth, activeWidth, margin } = calculateCardWidth();

        cards.forEach((card, index) => {
            card.classList.toggle('active', index === currentIndex);
            card.style.width = index === currentIndex ? `${activeWidth}px` : `${cardWidth}px`;
    ...
    track.addEventListener('touchmove', (e) => {
        if (isDragging) {
            ...
            track.style.transform = `translateX(${deltaX - currentIndex * (calculateCardWidth().cardWidth + calculateCardWidth().margin)}px)`;
        }
    });
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    sliders.forEach((slider, sliderIndex) => {
        const sliderWrapper = slider.querySelector('.cottages__images__slider');
        ...
        const updateImage = (index) => {
            // Рассчитываем смещение контейнера с изображениями
